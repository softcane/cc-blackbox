...
/// runs `cc-blackbox watch --tmux` and lands directly in the orchestrator.
///
...
pub fn bootstrap_into_tmux(
    ...
    let session_name = format!("cc-blackbox-{}", std::process::id());
...
fn resolve_cli_path() -> String {
    std::env::current_exe()
        ...
        .unwrap_or_else(|| "cc-blackbox".into())
}
...
impl TmuxOrchestrator {
    ...
    fn render_status(&mut self) {
        ...
        println!("{}", "cc-blackbox watch (tmux mode)".bold());
...
mod tests {
    ...
    fn test_orchestrator(max_panes: usize) -> super::TmuxOrchestrator {
        super::TmuxOrchestrator {
            ...
            cli_path: "cc-blackbox".to_string(),
            no_cache: false,
            no_signals: false,
            postmortem: false,
    ...
    fn child_watch_command_preserves_session_url_and_visibility_flags() {
        assert_eq!(
            ...
            "cc-blackbox watch --session session_a --url http://localhost:9091"
        );
